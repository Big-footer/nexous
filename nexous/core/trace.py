"""
NEXOUS Core - Trace System

실행 기록을 저장하고 추적합니다.

플랫폼 책임:
- 실행 기록 저장 (Agent 호출, LLM 호출, Tool 호출)
- 토큰 사용량 추적
- 비용 계산
- 실행 시간 측정

주의: Trace는 "기록"만 합니다.
실제 작업은 ProjectAgent가 수행하고, 플랫폼은 그것을 기록합니다.
"""
...
class Trace(BaseModel):
    """
    실행 Trace (플랫폼이 관리하는 실행 기록)
    
    플랫폼은 ProjectAgent의 모든 활동을 기록합니다.
    이를 통해 실행 추적, 디버깅, 비용 분석이 가능합니다.
    """
    # 식별
    ...
    def start_agent(self, agent_name: str, input_data: Optional[Dict] = None) -> AgentCall:
        ...
        logger.debug(f"[TRACE] Agent started: {agent_name}")
        return call
    
    def finish_agent(self, agent_name: str, output_data: Optional[Dict] = None, error: Optional[str] = None):
        """Agent 실행 완료 기록"""
        for call in reversed(self.agent_calls):
            if call.agent_name == agent_name and call.status == TraceStatus.RUNNING:
                ...
                logger.debug(f"[TRACE] Agent finished: {agent_name} ({call.duration_ms}ms)")
                break
    
    def log_llm_call(self, provider: str, model: str, prompt_tokens: int = 0, 
                     completion_tokens: int = 0, latency_ms: int = 0, cost_usd: float = 0.0,
        ...
        logger.debug(f"[TRACE] LLM call: {provider}/{model} ({call.total_tokens} tokens)")
    ...
    def log_tool_call(self, tool_name: str, arguments: Dict[str, Any], 
        ...
        logger.debug(f"[TRACE] Tool call: {tool_name} ({duration_ms}ms)")
    ...
    def complete(self, outputs: Optional[Dict] = None, artifacts: Optional[List[str]] = None):
        ...
        logger.info(f"[TRACE] Completed: {self.trace_id} ({self.duration_ms}ms, {self.total_tokens} tokens)")
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "agents_used": list(set(c.agent_name for c in self.agent_calls)),
...
class TraceStore:
    """
    Trace 저장소 (플랫폼이 관리하는 기록 저장소)
    
    모든 실행 기록을 파일 시스템에 저장하고 조회합니다.
    """
    
    def __init__(self, base_dir: Union[str, Path] = None):
        self.base_dir = Path(base_dir) if base_dir else Path.home() / ".nexous" / "traces"
        self.base_dir.mkdir(parents=True, exist_ok=True)
        self._index_file = self.base_dir / "index.json"
        self._index: Dict[str, Dict] = self._load_index()
        logger.info(f"[NEXOUS] TraceStore initialized: {self.base_dir}")
    ...
    def save(self, trace: Trace) -> str:
        """Trace 저장"""
        trace_dir = self.base_dir / (trace.project_id or "default")
        ...
        self._save_index()
        logger.info(f"[NEXOUS] Trace saved: {trace.trace_id}")
    ...
    def list_by_project(self, project_id: str) -> List[Dict]:
        """프로젝트별 Trace 목록"""
        return [{**v, "trace_id": k} for k, v in self._index.items() if v.get("project_id") == project_id]
    
    def list_recent(self, limit: int = 20) -> List[Dict]:
        """최근 Trace 목록"""
        traces = [{**v, "trace_id": k} for k, v in self._index.items()]
        traces.sort(key=lambda x: x.get("started_at", ""), reverse=True)
        return traces[:limit]
    
    def get_stats(self) -> Dict[str, Any]:
        """저장소 통계"""
        total_tokens = sum(v.get("total_tokens", 0) for v in self._index.values())
        total_cost = sum(v.get("total_cost_usd", 0) for v in self._index.values())
        return {
            "total_traces": len(self._index),
...
def create_trace(project_id: Optional[str] = None, project_name: Optional[str] = None,
                 request: Optional[str] = None, config: Optional[Dict] = None) -> Trace:
    """새 Trace 생성"""
    return Trace(
